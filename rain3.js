...
/* ==========================================================================
   Fogged window + tilt — same glass as rain2, gravity from the phone.

   Device orientation / accelerometer (the tilt2 pipeline) aims the drop
   gravity vector. Wipe, fog, and optics stay as they were.
   ========================================================================== */
...
/* -------------------------------------------------------------- tilt input
   Same sensor mapping as tilt2: iOS uses deviceorientation (relative to the
   pose at activation), Android uses accelerationIncludingGravity. Desktop
   can fake it from the cursor. */
...
const calib = { ready: false, beta: 0, gamma: 0, ax: 0, ay: 0 };
...
function updateGravity() {
  ...
  if (usingDeviceTilt) {
    grav.x = tilt.x * params.tiltStrength;
    grav.y = tilt.y * params.tiltStrength;
    if (Math.hypot(grav.x, grav.y) < 0.2) grav.y = 1;
  } else if (params.useMouseTilt && !isTouch) {
    grav.x = tilt.x * params.tiltStrength;
    grav.y = tilt.y * params.tiltStrength + 0.15;
  } else {
    grav.x = 0;
    grav.y = 1;
  }
...
function gravDir() {
  const mag = Math.hypot(grav.x, grav.y) || 1;
  return { x: grav.x / mag, y: grav.y / mag };
}
...
layout(location=0) in vec2 aQuad;
layout(location=1) in vec4 aPosR;   // x, y (px), radius (px), stretch along fall
layout(location=2) in vec4 aVal;    // mass, taper, fall dir x/y (canvas, y+ down)
uniform vec2 uCanvas;
out vec2 vP;
out float vA;
out float vTaper;
...
void main(){
  ...
  dir = dlen > 1e-4 ? dir / dlen : vec2(0.0, 1.0);
  vec2 perp = vec2(dir.y, -dir.x);
  vec2 local = aQuad * vec2(1.0, aPosR.w);
  vec2 px = aPosR.xy + (perp * local.x + dir * local.y) * aPosR.z;
  gl_Position = vec4(px.x / uCanvas.x * 2.0 - 1.0, 1.0 - px.y / uCanvas.y * 2.0, 0.0, 1.0);
}`;
...
void main(){
  ...
  // Shadow and caustic land downhill of the drop, on pixels the drop itself
  // does not cover, so they are sampled outside the block above.
  vec2 gUv = vec2(uGrav.x, -uGrav.y);
  float gLen = length(gUv);
  gUv = gLen > 1e-4 ? gUv / gLen : vec2(0.0, -1.0);
  float hNear = texture(uDrops, suv + gUv * uTexel * 2.2).r;
  float hFar  = texture(uDrops, suv + gUv * uTexel * 4.5).r;
...
function updateDrops(dt, time) {
  ...
  for (let i = drops.length - 1; i >= 0; i--) {
    ...
    if (d.stuck) {
      ...
      if (d.r > release) {
        d.stuck = false;
        const kick = (20 + 40 * Math.random()) * dpr;
        d.vx = g.x * kick;
        d.vy = g.y * kick;
      }
      continue;
    ...
    let along = d.vx * g.x + d.vy * g.y;
    let perp = d.vx * px_ + d.vy * py_;
    along = Math.min(along + accel * dt * dpr, vMax);
    ...
    d.vx = along * g.x + perp * px_;
    d.vy = along * g.y + perp * py_;
  ...
  for (let i = 0; i < drops.length; i++) {
    ...
    for (let j = i + 1; j < drops.length; j++) {
      ...
      const br2 = big.r * big.r;
      const sr2 = small.r * small.r;
      big.vx = (big.vx * br2 + small.vx * sr2) / (br2 + sr2);
      big.vy = (big.vy * br2 + small.vy * sr2) / (br2 + sr2);
...
/* A clear cell with fog two cells downhill is the lip of a wipe —
...
function scanForDrips(t) {
  ...
  const dry = 1 / Math.max(params.refogDelay, 0.15);
  const g = gravDir();
  // grid row increases toward the top of the screen (canvas y decreases)
  let dc = Math.round(g.x * 2);
  let dr = Math.round(-g.y * 2);
  if (dc === 0 && dr === 0) dr = -2;
  const candidates = [];
  ...
  for (let col = 0; col < gridW; col++) {
    for (let row = 0; row < gridH; row++) {
      const br = row + dr;
      const bc = col + dc;
      if (br < 0 || br >= gridH || bc < 0 || bc >= gridW) continue;
  ...
  while (count-- > 0) {
    ...
    const speed = (25 + 55 * Math.random()) * dpr;
    spawnDrop(x, y, {
      rMin: 3.5,
      rVar: 5,
      vx: g.x * speed,
...
canvas.addEventListener("pointerdown", (e) => {
  hideHint();
  if (isTouch && !usingDeviceTilt) enableTilt();
  lastPointer = null;
...
window.addEventListener("pointermove", (e) => {
  ...
  if (e.pointerType !== "mouse" || lastPointer) {
    wipeTo(e.clientX * dpr, e.clientY * dpr);
  }
});
...
function renderDropHeights() {
  ...
  for (const d of drops) {
    ...
    dropData[o + 1] = d.y;
    dropData[o + 2] = d.r;
    dropData[o + 3] = 1 + Math.min(sp / (320 * dpr), 1.2);
...
function composite(time) {
  ...
  gl.uniform2f(u.uGrav, grav.x, grav.y);
...
function frame(t) {
  ...
  while (ambientAcc > 1) {
    ...
    if (Math.abs(g.y) >= Math.abs(g.x)) {
      x = Math.random() * W;
      y = g.y >= 0 ? Math.random() * H * 0.45 : H * (0.55 + Math.random() * 0.45);
    } else {
      y = Math.random() * H;
      x = g.x >= 0 ? Math.random() * W * 0.45 : W * (0.55 + Math.random() * 0.45);
    }
    const speed = (18 + 30 * Math.random()) * dpr;
    spawnDrop(x, y, {
      rMin: 5.5,
      rVar: 5.5,
      vx: g.x * speed,
...
function buildGui() {
  const gui = new GUI({ title: "Tilt rain" });
  ...
  const fTilt = gui.addFolder("Tilt");
  fTilt.add(params, "tiltStrength", 0.2, 4, 0.05).name("Tilt strength");
  fTilt.add(params, "tiltSensitivity", 8, 40, 1).name("Sensitivity");
  fTilt.add(params, "tiltSmooth", 0.02, 0.4, 0.01).name("Smoothing");
  fTilt.add(params, "useMouseTilt").name("Mouse tilt (desktop)");
  fTilt
    .add(
      {
        recalibrate: () => {
